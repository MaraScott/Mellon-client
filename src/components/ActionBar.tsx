...
export default function AppToolbar() {
    ...
    const { updateLocalStorage, activeHistory } = useNodeState();
    const { exportGraph } = useNodeState((state) => ({ exportGraph: state.exportGraph }), shallow);
    const { undo, redo, past, future, present } = useUndoRedoStore();
    const { sid, isConnected } = useWebsocketState((state) => ({ sid: state.sid, isConnected: state.isConnected }), shallow);
    ...
    const onUndo = useCallback(() => {
        undo();
        const presentState = useUndoRedoStore.getState().present;
